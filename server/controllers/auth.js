...
exports.signup = function(req, res) {
  if (!req.body.email || !req.body.password) {
    ...
    try {
      pool.query('SELECT * from users where email = $1', [req.body.email], async function(
        err,
        rows,
      ) {
        if (err) res.send(err);
...
exports.login = function(req, res) {
  if (!req.body.email) {
    ...
    pool.query('SELECT * from users where email = $1', [req.body.email], async function(err, rows) {
      if (err) throw err;
      if (rows && rows.rowCount === 0) {
        ...
        const passwordValid = await compare(req.body.password, rows.rows[0].password);
        if (!passwordValid) {
